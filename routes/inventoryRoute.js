...
app.get("/view", shopauth, (req, res) => {
    (async () => {
        try {
            ...
            var shop = await shopServices.getShopbyId(req.session.sid)
            var orderPromise = await orderServices.getOrder(params);

            res.render('inventory', { products: productPromise, subCategories: subcatPromise, categories: catPromise, shop,order:orderPromise })
        } catch (error) {
            console.log(error)
        }
    })();
